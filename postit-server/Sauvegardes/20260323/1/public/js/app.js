...
async function loadGroupData(groupId) {
    ...
    // 2. Charger les Clients avec tri Chronologique Complet
    if (selDev.value) {
        ...
        if (postits.length === 0) {
            ...
            selPos.innerHTML = '<option value="">-- Client --</option>' + 
                postits.map(p => {
                    ...
                    // On concatène Date + Heure + Nom
                    return `<option value="${p._id}">[${dateShort} ${time}] ${truncate(p.name, 20)}</option>`;
                }).join('');
        }
    }
    refreshView();
