...
async function loadGroupData(groupId) {
    ...
    // MODIFICATION ICI : On enlève le "-- Rayon --" pour mettre direct les vrais rayons
    if (devs.length === 0) {
    ...
    // On garde ta logique de conservation de sélection si elle existe déjà
    ...
    // 2. Charger les Clients (le reste de ta fonction ne change pas)
    if (selDev.value) {
...
async function refreshView() {
    ...
    let headerHtml = "";
    if (pid && pSel.selectedIndex > 0) {
        try {
            const res = await fetch(`/api/postits/details/${pid}`);
            const p = await res.json();
            
            if (p) {
                let dateDisplay = "Non planifié";
                if (p.pickupDate) {
                    const d = new Date(p.pickupDate);
                    // MODIFICATION : On ne garde que l'heure et les minutes
                    dateDisplay = d.toLocaleTimeString('fr-FR', { 
                        hour: '2-digit', 
                        minute: '2-digit' 
                    });
                }
                ...
                headerHtml = `
                <div class="mb-4 p-3 border-2 border-black bg-white shadow-[4px_4px_0px_#000] text-[10px]">
                    <div class="flex justify-between items-start mb-2">
                        <span class="font-black uppercase text-xs italic">📦 ${p.orderNumber || 'SANS N°'}</span>
                        <span class="bg-black text-white px-2 py-0.5 font-bold uppercase tracking-tighter">${p.status || 'EN ATTENTE'}</span>
                    </div>
                    <div class="space-y-1 mt-2">
                        <div class="flex items-center gap-2">
                            <span class="font-black px-1 bg-red-600 text-white uppercase">RDV:</span> 
                            <span class="font-bold text-sm">${dateDisplay}</span>
                        </div>
                        <div class="opacity-70"><span class="font-black uppercase">Client:</span> ${p.name}</div>
                        <div class="opacity-70"><span class="font-black uppercase">Tel:</span> ${p.phone || '-'}</div>
                    </div>
                </div>`;
            }
