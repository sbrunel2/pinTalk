...
async function loadGroups() {
    ...
    // 1. Remplissage du menu déroulant (ton code existant)
    if (groups.length > 0) {
        sel.innerHTML = groups.map(g => `<option value="${g._id}">${truncate(g.name, 30)}</option>`).join('');
        // On ne force syncSelection que si c'est nécessaire
        if (!sel.value) syncSelection('group', groups[0]._id);
    }

    // 2. AJOUT : Remplissage de la liste dans l'onglet Paramètres
    // On utilise l'ID "list-groups-del" que tu as déjà dans ton HTML
