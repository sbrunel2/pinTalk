...
function initApp() {
	...
	socket.on('line-checked-updated', (data) => {
		...
		if (msg) {
			...
			refreshView(false); // On redessine sans scroller
		}
	});
    loadGroups();
}
...
async function refreshView(forceScrollBottom = false) {
    ...
    // --- FILTRAGE SIMULATEUR E-INK (On masque les notes) ---
	...
	if (einkFull) einkFull.innerHTML = einkHtml;    let headerHtml = "";
    let prepHeaderHtml = "";
    ...
    if (chat) {
        ...
        chat.innerHTML = (pid ? headerHtml : "") + [...filtered].reverse().map(m => {
            ...
            // --- INVERSION DES STYLES ---
            // Si c'est MOI (bulle foncée) -> Tag BLANC sur NOIR
            // Si c'est AUTRE (bulle blanche) -> Tag NOIR sur BLANC
