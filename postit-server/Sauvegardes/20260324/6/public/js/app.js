...
function toggleLineCheck(messageId) {
    ...
    // Récupérer le statut actuel pour ne pas écraser "Annulé" si on clique par mégarde
    // (Optionnel : si tu veux que cocher une ligne sorte de l'état "Annulé", garde le code tel quel)
...
async function refreshView(forceScrollBottom = false) {
    ...
    const einkHtml = forEink.map(m => {
        ...
        return `
        <div class="flex items-center gap-3 mb-2 group cursor-pointer" onclick="event.stopPropagation(); toggleLineCheck('${m._id}')">
    ...
    if (pid && pid !== "") {
        try {
            ...
            if (p) {
                ...
                // --- VERSION ULTRA COMPACTE ET FIXE ---
				const getStatusSelect = (fontSizeClass) => `
					<button onclick="event.stopPropagation(); showStatusMenu(this, '${p._id}')" 
...
function showStatusMenu(btn, pid) {
    // On supprime un éventuel menu déjà ouvert
    const oldMenu = document.getElementById('status-popup');
    if (oldMenu) oldMenu.remove();

    const rect = btn.getBoundingClientRect();
    const menu = document.createElement('div');
    menu.id = 'status-popup';
    // Style du menu volant (Look Neo-Brutalist)
    menu.className = "fixed z-[100] bg-white border-2 border-black shadow-[4px_4px_0px_#000] flex flex-col w-[120px]";
    menu.style.top = `${rect.top + window.scrollY - 5}px`;
    menu.style.left = `${rect.left + window.scrollX - 10}px`;

    const options = [
        { label: "⏳ ATTENTE", value: "En attente", bg: "bg-black" },
        { label: "🔥 PRÉPA.", value: "En préparation", bg: "bg-orange-500" },
        { label: "✅ TERMINÉ", value: "Terminé", bg: "bg-green-600" },
        { label: "❌ ANNULÉ", value: "Annulé", bg: "bg-gray-500" }
    ];

    menu.innerHTML = options.map(opt => `
        <button onclick="execChangeStatus('${pid}', '${opt.value}')" 
                class="px-2 py-2 text-[10px] font-black uppercase border-b border-black last:border-0 hover:bg-gray-100 text-left flex items-center gap-2">
            <span class="w-2 h-2 ${opt.bg} border border-black"></span>
            ${opt.label}
        </button>
    `).join('');
...
function execChangeStatus(pid, newStatus) {
    let cancelReason = "";
    if (newStatus === "Annulé") {
        cancelReason = prompt("Motif de l'annulation (obligatoire) :");
