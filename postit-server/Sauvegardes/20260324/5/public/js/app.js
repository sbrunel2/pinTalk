...
function toggleLineCheck(messageId) {
    // 1. Trouver le message dans la liste locale
    const msg = allMsgs.find(m => m._id === messageId);
    if (!msg) return;

    // 2. Inverser la coche LOCALEMENT pour le calcul immédiat
    msg.checked = !msg.checked;

    // 3. Envoyer l'ordre au serveur pour la base de données
    socket.emit('toggle-check-line', { messageId });

    // 4. Calculer le nouveau statut basé sur l'état actuel de TOUTES les lignes
    const pSel = document.getElementById('sel-pos');
    ...
    // 5. Envoyer le nouveau statut au serveur
    socket.emit('update-postit-status', { postitId: pid, status: newStatus });

    // 6. Rafraîchir l'affichage immédiatement
    refreshView(false);
}

function changeStatusManually(pid) {
    const currentStatus = document.getElementById('current-status-text').innerText;
    const states = ["En attente", "En préparation", "Terminé"];
    
    // Simple menu prompt pour choisir (ou tu peux faire un petit menu HTML personnalisé)
    const nextStatus = prompt(`Modifier le statut (actuel: ${currentStatus}) :\n1. En attente\n2. En préparation\n3. Terminé`);
    
    if (nextStatus === "1") socket.emit('update-postit-status', { postitId: pid, status: "En attente" });
    if (nextStatus === "2") socket.emit('update-postit-status', { postitId: pid, status: "En préparation" });
    if (nextStatus === "3") socket.emit('update-postit-status', { postitId: pid, status: "Terminé" });
}
...
async function refreshView(forceScrollBottom = false) {
    ...
    // --- FILTRAGE SIMULATEUR E-INK (On masque les notes et on ajoute les coches) ---
    const forEink = allMsgs.filter(m => m.postitId === pid && !m.isNote);
    const einkHtml = forEink.map(m => {
        // Style de la case selon l'état
        const boxClass = m.checked 
            ? "bg-green-500 border-black text-white" 
            : "bg-white border-black text-transparent";
        
        const textStyle = m.checked 
            ? "color: #a1a1aa; text-decoration: line-through;" 
            : "color: #000;";

        return `
        <div class="flex items-center gap-3 mb-2 group cursor-pointer" 
             onclick="event.stopPropagation(); toggleLineCheck('${m._id}')">
            
            <div class="w-5 h-5 border-2 flex-shrink-0 flex items-center justify-center transition-colors ${boxClass}">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">
                    <polyline points="20 6 9 17 4 12"></polyline>
                </svg>
            </div>

            <span class="text-[13px] font-bold leading-none" style="${textStyle}">
                ${m.content}
            </span>
        </div>`;
    }).join('');
    ...
    if (pid && pid !== "") {
        try {
            ...
            const p = await res.json();
            if (p) {
                // Définition de la couleur du badge selon le statut
                let statusBg = "bg-black"; // Par défaut "En attente"
                ...
                if (p.pickupDate) {
                    ...
                    } else { formattedDate = p.pickupDate; }
                }

                // HEADER RÉSUMÉ (Sous la simulation)
                headerHtml = `
                <div class="p-3 border-4 border-black bg-white shadow-[4px_4px_0px_#000] mb-4">
                    <div class="flex justify-between items-start border-b-2 border-black pb-2 mb-2">
                        ...
                        <button onclick="changeStatusManually('${p._id}')" class="${statusBg} text-white px-2 py-1 font-bold uppercase text-[10px] border-2 border-black active:translate-y-[1px]">
                            ${p.status || 'En attente'}
                        </button>
                    </div>
                ...
                // HEADER ÉCRAN PRÉPARATION (Agrandi)
                prepHeaderHtml = `
                <div class="p-3 border-4 border-black bg-white shadow-[4px_4px_0px_#000]">
                    <div class="flex justify-between items-start border-b-2 border-black pb-2 mb-2">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-[10px] font-black uppercase opacity-40">Statut actuel</span>
                                <button onclick="changeStatusManually('${p._id}')" class="${statusBg} text-white px-2 py-0.5 font-bold uppercase text-[9px] border border-black">
                                    ${p.status || 'En attente'}
                                </button>
                            </div>
            ...
            <div class="msg-row ${isMe ? 'me' : 'others'} ${noteClass} mb-2">
                <div class="msg-bubble relative p-1.5 border-2 border-black ${bubbleBg} shadow-[2px_2px_0px_#000]">
                    <div class="flex items-center gap-2">
                        <span class="text-[8px] font-black px-1.5 py-0.5 uppercase flex-shrink-0 ${tagStyle} leading-tight">
                            ${isMe ? 'Moi' : m.senderName}
                        </span>
                        <span class="text-[13px] font-bold leading-tight flex-1">${m.content}</span>
                        <button onclick="toggleNote('${m._id}')" class="ml-1 flex-shrink-0 text-[14px] cursor-pointer grayscale">
                            ${m.isNote ? '🚫' : '👁️'}
                        </button>
                    </div>
                </div>
