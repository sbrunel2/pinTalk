...
function toggleLineCheck(messageId) {
    // On récupère le bouton de statut pour vérifier l'état actuel
    const btn = document.getElementById('btn-status-main');
    if (btn && btn.getAttribute('data-status') === "Annulé") {
        console.warn("Action bloquée : Commande annulée");
        return;
    }
    ...
    // Mise à jour auto du statut basée sur les coches (on garde ta logique stable)
    const pSel = document.getElementById('sel-pos');
...
async function refreshView(forceScrollBottom = false) {
    ...
    let headerHtml = "";
    let prepHeaderHtml = "";
    let currentStatus = ""; // On stocke le statut pour le verrouillage des lignes
    ...
    if (pid && pid !== "") {
        try {
            ...
            if (p) {
                ...
                // --- RECHERCHE DU MOTIF D'ANNULATION ---
    ...
    const einkHtml = forEink.map(m => {
        ...
        const opacityClass = isLocked ? "opacity-40 cursor-not-allowed" : "cursor-pointer";
        
        return `
        <div class="flex items-center gap-3 mb-2 group ${opacityClass}" 
             onclick="${isLocked ? '' : `event.stopPropagation(); toggleLineCheck('${m._id}')`}">
