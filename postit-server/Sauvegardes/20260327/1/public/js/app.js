...
async function uiCreateGroup(e) {
    ...
    // 1. On récupère l'utilisateur AVANT d'ouvrir la fenêtre (Sécurité V3)
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user || !user.email) return alert("Erreur : session expirée. Reconnectez-vous.");

    // 2. On utilise la fenêtre HTML à la place du prompt
    openCustomPrompt("Nom du nouveau groupe (ex: BOUCHERIE, CUISINE...)", "", async (name) => {
        ...
        try {
            const res = await fetch('/api/groups', {
                ...
                body: JSON.stringify({ 
                    name: name.trim(), 
                    ownerEmail: user.email // Ta sécurité V3 est préservée
                })
            });
            ...
            if (res.ok) {
                ...
                setTimeout(async () => {
                    await refreshParamsLists(); // On s'assure que la liste des réglages est à jour
...
async function uiCreateDevice(e) {
    ...
    const gid = selGroup ? selGroup.value : null;
    if(!gid) return alert("Sélectionnez d'abord un groupe");

    // 2. RÉCUPÉRATION DE L'UTILISATEUR (Indispensable pour la V3)
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user || !user.email) return alert("Erreur : session expirée. Reconnectez-vous.");
        ...
        try {
            const res = await fetch('/api/devices', { 
                ...
                body: JSON.stringify({ 
                    ...
                    name: name.trim(), 
                    mac: "00",
                    ownerEmail: user.email // <--- AJOUTÉ : Pour lier le rayon à l'utilisateur
                })
            });

            if(res.ok) {
                const data = await res.json();
                lastCreatedId = data._id; // On garde ton effet visuel

                // Mise à jour des données du chat
                await loadGroupData(gid); 
                
                // Rafraîchissement propre de l'interface des réglages
                setTimeout(async () => {
    ...
    container.innerHTML = (items || []).map(item => {
		...
		<div class="flex items-center p-3 mb-1 ${isSelected ? 'bg-black/5 font-black' : 'opacity-50'} ${flashClass}">
			...
			<div class="flex items-center gap-1">
				<button onclick="event.stopPropagation(); editName('${type}', '${item._id}', '${item.name}')" class="btn-edit">🖍️</button>
				<button 
...
async function refreshParamsLists() {
    // 1. Récupération de l'utilisateur pour la sécurité V3
    const user = JSON.parse(localStorage.getItem('user'));
    ...
    // 4. Chargement des POST-ITS (avec email + deviceId)
    if (currentDid) {
        // Note: On ajoute aussi l'email ici pour préparer la sécurité des Post-its
...
/*async function refreshParamsLists() {
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user || !user.email) return console.error("🛑 Pas d'utilisateur en localStorage");
    
    const emailParam = `email=${encodeURIComponent(user.email)}`;
    const currentGid = document.getElementById('sel-group').value;
    
    console.log("🔍 [DEBUG] GID actuel:", currentGid);
    ...
    // 3. RAYONS (Le point de blocage)
    if (currentGid) {
        const urlD = `/api/devices?groupId=${currentGid}&${emailParam}`;
        console.log("📡 [DEBUG] Fetch Rayons URL:", urlD);
        
        const dRes = await fetch(urlD);
        const devs = await dRes.json();
        
        console.log("📦 [DEBUG] Rayons reçus du serveur:", devs);
        ...
        console.warn("⚠️ Aucun GroupID sélectionné, arrêt du refresh");
    }
}*/
...
async function loadGroups() {
    // 1. RÉCUPÉRATION DE L'UTILISATEUR (Sécurité V3)
    const user = JSON.parse(localStorage.getItem('user'));
    if (!user || !user.email) return;

    try {
        // 2. FETCH FILTRÉ (On envoie l'email au serveur)
        const res = await fetch(`/api/groups?email=${encodeURIComponent(user.email)}`);
        const groups = await res.json();
        
        const sel = document.getElementById('sel-group');
        
        // 3. REMPLISSAGE DU MENU DÉROULANT (Ton code)
        if (sel && groups.length > 0) {
            sel.innerHTML = groups.map(g => `<option value="${g._id}">${g.name}</option>`).join('');
            if (!sel.value) syncSelection('group', groups[0]._id);
        } else if (sel) {
            sel.innerHTML = '<option value="">Aucun groupe</option>';
        }

        // 4. REMPLISSAGE DE LA LISTE DANS LES PARAMÈTRES (Ton code uniformisé)
        const listDiv = document.getElementById('list-groups-del');
        if (listDiv) {
            listDiv.innerHTML = groups.map(g => `
                <div class="flex justify-between items-center p-2 border-b border-black/10 text-[10px] font-black uppercase">
                    <span>${g.name}</span>
        ...
        console.error("Erreur chargement groupes front:", err);
    }
}
...
async function deleteGroup(id) {
    // Plus besoin de confirm() ici, car le bouton "OK?" gère la sécurité
    try {
        ...
        if (res.ok) {
            // 1. On met à jour la liste globale (le menu du chat)
            await loadGroups();
            
            // 2. On rafraîchit l'interface des réglages après un court délai
            setTimeout(async () => {
                await refreshParamsLists();
                // On s'assure que l'accordéon "GROUPES" reste ouvert
                const checkG = document.getElementById('check-g');
                if (checkG) checkG.checked = true;
            }, 300);
        } else {
        ...
        console.error("Erreur réseau deleteGroup:", err);
...
async function deleteDevice(id) {
    try {
        ...
        if (res.ok) {
            const currentGid = document.getElementById('sel-group').value;
            // Rafraîchissement des données et de l'interface
