...
async function initApp() {
    ...
    // Annuler le mode édition si tap sur le chat en dehors d'une bulle en cours d'édition
    document.addEventListener('touchstart', (e) => {
        ...
        // Ignorer si tap sur la bulle en cours, msg-input, ou le bandeau
        if (swipeEl?.contains(e.target)) return;
        if (msgBar?.contains(e.target)) return;
        if (document.getElementById('edit-mode-banner')?.contains(e.target)) return;
        if (document.getElementById('selection-context-menu')?.contains(e.target)) return;
