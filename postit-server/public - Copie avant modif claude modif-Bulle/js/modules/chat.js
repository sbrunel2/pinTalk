...
async function send() {
    ...
    // ── Mode édition : PATCH au lieu d'envoi ─────────────────────────────────
    if (window._editingMessageId) {
        const editId = window._editingMessageId;
        const msg    = allMsgs.find(m => m._id === editId);
        if (msg && text !== msg.content) {
            try {
                const res = await fetchAuth('/api/messages/' + editId, {
                    method: 'PATCH',
                    body: JSON.stringify({ content: text })
                });
                if (res.ok) {
                    const pid2 = msg.postitId;
                    msg.content = text;
                    if (pid2 && msg.senderName !== '✨ IA' && !msg.isNote) {
                        await _deleteAiNotesForMessage(editId, pid2);
                        setTimeout(() => aiAutoExtract(text, pid2, editId), 200);
                    }
                }
            } catch(e) { console.error('Edit PATCH:', e); }
        }
