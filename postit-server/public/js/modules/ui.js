function _initSelectionMenu() {
    ...
    const showMenu = (text) => {
        ...
        if (vv && vv.height < window.innerHeight * 0.85) {
            // Clavier ouvert : positionner juste au-dessus du clavier
            const keyboardTop = vv.offsetTop + vv.height;
            menu.style.top    = (keyboardTop - 36) + 'px';
            menu.style.bottom = 'auto';
        } else {
            // Pas de clavier : au-dessus du bandeau orange ou de la tab-bar
            ...
            if (banner) {
                ...
                menu.style.bottom = (tabBar ? tabBar.getBoundingClientRect().height : 56) + 'px';
            }
            menu.style.top = 'auto';
        }
    ...
    // Repositionner si le clavier bouge
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', () => {
            if (_menuVisible) showMenu(_lastSelection);
        });
    }
    ...
    document.getElementById('smenu-quote').addEventListener('click', () => {
        ...
        // Lire le texte courant : textarea inline si en mode édition, sinon msg.content
        const ta = window._editingTextarea;
        const currentText = (ta && window._editingMessageId === msgId)
            ? ta.value
            : (allMsgs.find(m => m._id === msgId)?.content || '');

        if (!currentText) return;
        const newContent = currentText.replace(phrase, '\u201c' + phrase + '\u201d');

        if (ta && window._editingMessageId === msgId) {
            // Déjà en mode édition sur cette bulle → juste mettre à jour le textarea
            ta.value = newContent;
            ta.dispatchEvent(new Event('input')); // trigger auto-resize
        } else {
            // Entrer en mode édition avec le texte modifié
            if (typeof editMessage === 'function') editMessage(msgId, newContent);
        }
    });
}
...
function editMessage(id, initialText = null) {
    ...
    // Désactiver le swipe-to-reveal pendant l'édition
    if (swipeEl) {
    ...
    // ── Clé iOS : effacer la sélection AVANT toute manipulation DOM ──────────
    window.getSelection()?.removeAllRanges();

    // Attendre 200ms que iOS traite le removeAllRanges avant d'insérer l'input
    setTimeout(() => {
        if (window._editingMessageId !== id) return; // annulé entre-temps

        const originalText = initialText !== null ? initialText : msg.content;

        // Créer un textarea (pas input) pour gérer le texte multilignes
        ...
        ta.style.cssText = `
            ...
            padding:4px 6px; font-size:inherit;
            font-family:inherit; font-weight:inherit; line-height:inherit;
            border-radius:2px; resize:none; overflow:hidden;
            min-width:60px;
        `;
        ...
        const save = async () => {
            ...
            } catch(e) { console.error('editMessage save:', e); }
        };

        ta.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); ta.blur(); }
            if (e.key === 'Escape') { _cancelEditMode(); }
        });
        ta.addEventListener('blur', save);

        // Masquer le span, insérer le textarea à sa place
        textSpan.style.display = 'none';
        textSpan.parentNode.insertBefore(ta, textSpan);

        // Stocker référence pour _cancelEditMode
        window._editingTextarea = ta;
        window._editingTextSpan = textSpan;
        ...
        // Scroller vers la bulle
        if (swipeEl) swipeEl.scrollIntoView({ behavior: 'smooth', block: 'end' });
...
function _cancelEditMode() {
    ...
    // Restaurer l'outline de la bulle
    if (swipeEl) {
        swipeEl.style.outline       = swipeEl.dataset.prevOutline || '';
        swipeEl.style.outlineOffset = '';
    }

    // Retirer le textarea et restaurer le span
    const ta       = window._editingTextarea;
    const textSpan = window._editingTextSpan;
    if (ta && ta.parentNode) ta.parentNode.removeChild(ta);
    if (textSpan) textSpan.style.display = '';

    // Nettoyer l'ancien mode msg-input si présent (compat arrière)
    const msgInput = document.getElementById('msg-input');
    if (msgInput) msgInput.style.outline = '';

    window._editingMessageId    = null;
    window._editingOriginalText = null;
    window._editingTextarea     = null;
    window._editingTextSpan     = null;
